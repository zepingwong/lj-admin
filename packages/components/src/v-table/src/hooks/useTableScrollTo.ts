...
import { warn } from '@lj/utils'
import { ComponentRef } from '@lj/types/src/common'
...
export function useTableScrollTo(tableElRef: Ref<ComponentRef>, getDataSourceRef: ComputedRef<Recordable[]>) {
  ...
  function scrollTo(pos: string): void {
    ...
    // judge pos type
    if (pos === 'top') {
      findTargetRowToScroll(dataSource[0]).catch()
    } else if (pos === 'bottom') {
      findTargetRowToScroll(dataSource[dataSource.length - 1]).catch()
    } else {
      const targetRowData = dataSource.find((data) => data.id === pos)
      if (targetRowData) {
        findTargetRowToScroll(targetRowData).catch()
      } else {
